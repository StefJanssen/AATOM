 ...
 */
public class BasicRegionalCheckpointActivity extends CheckpointActivity {
	...
	private double remainingDropTime = 0;
	 ...
	 * 
	 */
	public BasicRegionalCheckpointActivity(Flight flight, MathDistribution dropDistribution,
	...
	public boolean canStart(int timeStep) {
		...
		if (movement.getPosition().distanceTo(activityPosition) < 0.5) {
			...
			for (XRaySystem system : systems) {
				...
				if (system.getDropOffPassenger(system.getNumberOfDropoffIndices() - 1) == null) {
					// check how many people are there
					int tmpNumberOfPeople = 0;
					for (int i = 0; i < system.getNumberOfDropoffIndices(); i++) {
						if (system.getDropOffPassenger(i) != null)
							tmpNumberOfPeople++;
					}
					if (tmpNumberOfPeople < numberOfPeople) {
						currentBest = system;
			...
			if (currentBest != null) {
				xRaySystem = currentBest;
				dropOffPosition = xRaySystem.getNumberOfDropoffIndices() - 1;
	...
	private void dropLuggageAndgoToWTMD() {
		...
		// moving forward in the belt
		if (dropOffPosition > 0 && phase == 1) {
			if (xRaySystem.getDropOffPassenger(dropOffPosition - 1) == null) {
				remainingDropTime = movement.getStopMovingTime();
				movement.setStopOrder(0);
				Position p = xRaySystem.getDropOffPosition(dropOffPosition - 1);
				navigationModule.setGoal(p);
				xRaySystem.setDropOffPassenger((Passenger) agent, dropOffPosition - 1);
				xRaySystem.setDropOffPassenger(null, dropOffPosition);
				dropOffPosition--;
			}
		}

		// when moved forward and still time to drop left
		if (phase == 1 && navigationModule.getReachedGoal() && remainingDropTime > 0 && !movement.getStopOrder()) {
			movement.setStopOrder(remainingDropTime);
			remainingDropTime = 0;
		}

		// when not waiting anymore
		if (!movement.getStopOrder() && dropOffPosition == 0 && phase == 1
